...
// impl<'a> Cacheable<'a, CardDeckIdSetMap> for CardDeckIdSetMapReader<'_> {
...
impl Store<'_> {
    pub async fn card_deck_id_set_map(self: &Self) -> Result<()> {
        todo!()
        // CardDeckIdSetMapReader(self).load_or_compute(&self.cache).await
    }
}